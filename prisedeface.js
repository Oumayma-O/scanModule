...
function validrightarm(canvasCtx,mpPose,results,drawingUtils) {
    drawCircle(canvasCtx, results.poseLandmarks, [mpPose.POSE_LANDMARKS.RIGHT_SHOULDER ,mpPose.POSE_LANDMARKS.RIGHT_ELBOW ,mpPose.POSE_LANDMARKS.RIGHT_WRIST], { visibilityMin: 0.65, color: "#1CE496",fillColor: "#1CE496" });
    drawingUtils.drawConnectors(canvasCtx, results.poseLandmarks, [mpPose.POSE_CONNECTIONS[16],mpPose.POSE_CONNECTIONS[17]], { visibilityMin: 0.65, color: "#1CE496" });
}
function unvalidrightarm(canvasCtx,mpPose,results,drawingUtils,feedback)
{
    drawCircle(canvasCtx, results.poseLandmarks, [mpPose.POSE_LANDMARKS.RIGHT_SHOULDER ,mpPose.POSE_LANDMARKS.RIGHT_ELBOW ,mpPose.POSE_LANDMARKS.RIGHT_WRIST], { visibilityMin: 0.65, color: "#FF3D00",fillColor: "#FF3D00" });
    drawingUtils.drawConnectors(canvasCtx, results.poseLandmarks, [mpPose.POSE_CONNECTIONS[16],mpPose.POSE_CONNECTIONS[17]], { visibilityMin: 0.65, color: "#FF3D00" });
    feedback.innerHTML = "Lever le bras Droit";
}
function validarms(canvasCtx,mpPose,results,drawingUtils)
{       validrightarm(canvasCtx,mpPose,results,drawingUtils);
...
function unvisiblerightarm(canvasCtx,mpPose,results,drawingUtils,feedback)
{
    feedback.innerHTML = "Votre bras droit doit être entièrement visible !";
}
...
function onResults(results) {
    ...
    if (results.poseLandmarks) {
        ...
        if (A && B && C && D && E && F && AL && BL && CL && DL && EL && FL) {
            ...
            visiblebody= visibleleftarm && visiblerightarm && visibleleftleg && visiblerightleg;

            // Hand is a straight line
            angleLeftWrist=calculateAngle(C, B, E)-90
            if (Math.abs(angleLeftWrist) > 80 &&Math.abs(angleLeftWrist) < 95 && visibleleftarm) { okLeftWrist = true;}
            angleRightWrist=calculateAngle(D, A, F)-90
            if (Math.abs(angleRightWrist) > 80 && Math.abs(angleRightWrist) < 95 && visiblerightarm) { okRightWrist = true;}

            // Arms form an A Shape
            angleLeftArm=calculateAngle(B, A, C)-90
            if (angleLeftArm > 14.5 && visibleleftarm && okLeftWrist) { okLeftArm = true;}
            angleRightArm=calculateAngle(A, B, D)-90
            if (angleRightArm > 9 && visiblerightarm && okRightWrist) { okRightArm = true;}

            //Legs in a straight line
            angleLeftAnkle=calculateAngle(EL, FL, CL)-90;
            if (Math.abs(angleLeftAnkle) > 80 && visibleleftleg) { okLeftAnkle = true;}
            angleRightAnkle=calculateAngle(FL, EL, DL)-90;
            if (Math.abs(angleRightAnkle) > 80 && visiblerightleg) { okRightAnkle = true;}

            //Legs form a V Shape
            angleLeftLeg=calculateAngle(DL, BL, CL)-90;
            if (Math.abs(angleLeftLeg) > 55 &&visibleleftleg  ) { okLeftLeg = true;}
            angleRightLeg=calculateAngle(CL, AL, DL)-90;
            if (Math.abs(angleRightLeg) > 55 && visiblerightleg ) { okRightLeg = true;}
    ...
    if (results.poseWorldLandmarks) {
        ...
        if (visiblebody) {
            ...
            if (okRightArm && okLeftArm && okRightLeg && okLeftLeg) {
                ...
                if (okRightArm && okLeftArm) {
                    ...
                    if(okRightArm){
                        ...
                        unvalidrightarm(canvasCtx,mpPose,results,drawingUtils,feedback);
                    }
                    if(okLeftArm){
                        validleftarm(canvasCtx,mpPose,results,drawingUtils);
                    }
            ...
            unvisiblebody(canvasCtx,mpPose,results,drawingUtils);
            if(!visibleleftarm){
                unvisibleleftarm(canvasCtx,mpPose,results,drawingUtils,feedback);
            }
            if(!visiblerightarm){
                unvisiblerightarm(canvasCtx,mpPose,results,drawingUtils,feedback);
            }
            if(!visibleleftleg){
                unvisibleleftleg(canvasCtx,mpPose,results,drawingUtils,feedback);
            }
            if(!visiblerightleg){
                unvisiblerightleg(canvasCtx,mpPose,results,drawingUtils,feedback);
            }
